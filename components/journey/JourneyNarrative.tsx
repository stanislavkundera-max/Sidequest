...
type Props = {
  ...
  uiScale: number;
  /** Omit to render the pill as static (non-interactive) text. */
  onPress?: () => void;
};
...
export function JourneyNarrative({ line, contentWidth, uiScale, onPress }: Props) {
  ...
  const pillDynamicStyle = { maxWidth: maxW, paddingHorizontal: padH, paddingVertical: padV };
    ...
    <View style={[styles.wrap, { left: padH, right: padH, bottom: Math.round(14 + 4 * uiScale) }]} accessibilityLiveRegion="polite">
      {onPress ? (
        <Pressable
          ...
          style={({ pressed }) => [styles.pill, pillDynamicStyle, pressed && styles.pillPressed]}>
          <Text style={[styles.text, { fontSize, lineHeight }]}>{line}</Text>
        </Pressable>
      ) : (
        <View style={[styles.pill, pillDynamicStyle]}>
          <Text style={[styles.text, { fontSize, lineHeight }]}>{line}</Text>
        </View>
      )}
    </View>
